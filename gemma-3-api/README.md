...
- **Text-to-speech** through the OpenAI-compatible Higgs Audio V2 deployment with blocking and streaming responses.
- **Speech dialogue** endpoint that combines Whisper, Gemma, and Higgs Audio with optional streaming JSON events.
...
## Configuring Higgs Audio V2

1. Clone and start the Higgs Audio V2 service following the upstream instructions: <https://github.com/boson-ai/higgs-audio>.
2. Expose the service on an HTTP endpoint reachable by the Gemma API (for example `http://localhost:8000/v1`).
3. Set `HIGGS_API_BASE` to the base URL of the deployment and provide an API key via `HIGGS_API_KEY` if required.

The API expects an OpenAI-compatible `/audio/speech` route as provided by Higgs Audio V2. Streaming synthesis requires the upstream deployment to support HTTP chunked responses.
...
| `HIGGS_API_KEY` | Authentication token for Higgs Audio V2 deployments. |
| `HIGGS_API_BASE` | Base URL of the Higgs Audio API (e.g. `http://higgs_audio:8000/v1`). |
| `HIGGS_SPEECH_PATH` | Speech synthesis path appended to the base URL (defaults to `/audio/speech`). |
| `HIGGS_MODEL_ID` | Default Higgs Audio model identifier. |
| `HIGGS_DEFAULT_VOICE` | Default voice preset requested from Higgs Audio. |
| `HIGGS_RESPONSE_FORMAT` | Default output format (e.g. `pcm`, `wav`, `mp3`). |
...
To connect to a locally running Higgs Audio deployment inside the same Docker network, add a service definition (see upstream repository for a supported image) and update `HIGGS_API_BASE` to reference that service, e.g. `http://higgs_audio:8000/v1`.
...
| `POST` | `/v1/text-to-speech` | Convert text to speech using Higgs Audio. |
| `POST` | `/v1/dialogue` | Run end-to-end speech dialogue (Whisper → Gemma → Higgs Audio). |
...
1. `{"event": "metadata"}` — audio format, MIME type, sample rate, voice, and model information.
...
4. One or more `{"event": "audio_chunk"}` objects containing base64-encoded Higgs Audio samples.
...
- External integrations (Whisper, Higgs Audio, Gemma) contribute latency histograms, making it easy to identify bottlenecks.
